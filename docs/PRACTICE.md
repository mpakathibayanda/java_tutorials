...
public class Bank {
    ...
    public boolean removeAccount(String acctNumber) {
        // [12, 23, 34, 343, 12, 23]
        Iterator<Account> tempoAccts = accounts.iterator();
        while (tempoAccts.hasNext()) {
            Account nextAcct = tempoAccts.next();
            String accountNumer = tempoAccts.getAccountNumber();
            boolean accExist = (accountNumber == acctNumber);
            if (accExist) {
                tempoAccts.remove();
                return true;
            }
